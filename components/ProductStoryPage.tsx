  ...
  return <>
    <main>
      {featureStyleHero ? <section className="relative overflow-hidden bg-black px-6 pb-20 pt-36 text-white md:pb-24 md:pt-40">
        <div className="absolute inset-0 z-0 overflow-hidden"><div className="absolute inset-x-0 top-[-20%] h-[140%]"><Image src={heroImage} alt="" fill className="object-cover" priority sizes="100vw"/></div><div className="hero-image-overlay absolute inset-0"/><div className="hero-grain pointer-events-none absolute inset-0 opacity-30"/></div>
        ...
        <Image src={heroImage} alt="" fill priority className="object-cover object-center" sizes="100vw"/><div className="absolute inset-0 bg-linear-to-t from-black via-black/45 to-black/20"/><div aria-hidden className="hero-grain absolute inset-0"/><div className="relative mx-auto flex min-h-[calc(92svh-12rem)] max-w-[1440px] items-end"><div className="w-full"><div className="mb-8 flex flex-wrap gap-2"><span className="rounded-full border border-white/20 bg-black/35 px-4 py-2 text-xs font-medium backdrop-blur-md">Customers create demand</span><span className="rounded-full border border-white/20 bg-black/35 px-4 py-2 text-xs font-medium backdrop-blur-md">Specialists deliver skill</span><span className="rounded-full border border-white/20 bg-black/35 px-4 py-2 text-xs font-medium backdrop-blur-md">Vendors supply progress</span></div><MicroLabel>{eyebrow}</MicroLabel><h1 className="mt-4 max-w-4xl text-[clamp(2.75rem,7vw,6.5rem)] font-semibold leading-[.94] tracking-[-.045em]">{title}</h1><div className="mt-7 flex flex-col gap-7 md:flex-row md:items-end md:justify-between"><p className="max-w-xl text-base leading-relaxed text-white/70 md:text-lg">{intro}</p><div className="flex flex-wrap gap-3"><Link href={COMING_SOON_PATH} className="btn-premium inline-flex h-12 items-center justify-center rounded-full bg-[#FF5F15] px-7 text-sm font-semibold text-black hover:bg-[#FF7335] md:h-[52px] md:px-8 md:text-base">{primaryLabel}</Link><Link href={secondaryHref} className="btn-premium inline-flex h-12 items-center justify-center rounded-full border border-white/25 bg-black/30 px-7 text-sm font-semibold text-white backdrop-blur-md hover:bg-black/45 md:h-[52px] md:px-8 md:text-base">{secondaryLabel}</Link></div></div></div></div>
      </section>}
